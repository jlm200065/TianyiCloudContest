...
public class SysMovieController extends BaseController {
    ...
    public SysMovie findOneMovie(@PathVariable Long id) {
        return sysMovieService.findOneMovie(id);
    }
    ...
    public List<SysMovie> findRemoteAllMovies(SysMovieVo sysMovieVo) {
        List<SysMovie> data = sysMovieService.findAllMovies(sysMovieVo);
        return data;
    }

    @GetMapping("/sysMovie/remote/find/{id}")
    public SysMovie findReomoteMovieById(@PathVariable Long id) {
        return sysMovieService.findMovieById(id);
    }

    @GetMapping("/sysMovie/remote/findAll")
    public List<SysMovie> findRemoteAll(){
        return sysMovieService.findAll();
    }
